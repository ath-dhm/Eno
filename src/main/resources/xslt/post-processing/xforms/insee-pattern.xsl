    ...
    <xsl:template match="xf:submission[@id='save' or @id='submit']">
        <xsl:copy>
            ...
            <xsl:variable name="resource">
                <xsl:value-of select="concat('{xxf:property(''server-exist-orbeon'')}/apps/orbeon/{xxf:property(''enregistrer-service'')}/',$campaign-value,'/',$modele,'/{xxf:get-request-parameter(''unite-enquete'')}?ongletproof=oui')"/>
            </xsl:variable>
